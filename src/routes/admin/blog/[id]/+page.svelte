<script lang="ts">
  ...
  // Sync initial tag IDs into form
  $effect(() => {
  ...
  // Cover image - initialize from existing post data
  // svelte-ignore state_referenced_locally
  let coverPreviewUrl = $state<string | null>(data.post.coverUrl)
  let isCoverUploading = $state(false)
  // svelte-ignore state_referenced_locally
  let coverR2Key = $state<string | null>(data.post.coverR2Key)
  // svelte-ignore state_referenced_locally
  let coverR2KeyFull = $state<string | null>(data.post.coverR2KeyFull ?? null)
...
<div class="page">
  ...
  <form method="POST" action="?/update" use:enhance class="post-form">
    ...
    {#if coverR2KeyFull}<input
      ...
      />{/if}
    {#if data.post.coverR2Key}<input
        type="hidden"
        name="existingCoverR2Key"
        value={data.post.coverR2Key}
      />{/if}
    {#if data.post.coverR2KeyFull}<input
        type="hidden"
        name="existingCoverR2KeyFull"
        value={data.post.coverR2KeyFull}
      />{/if}
  ...
  <!-- Delete zone - outside the update form to avoid nested forms -->
...
<style>
  ...
  /* Danger zone */
  /* Danger zone - positioned to align with sidebar */
  ...
  .danger-zone-container .danger-zone {
    grid-column: 2;
  }

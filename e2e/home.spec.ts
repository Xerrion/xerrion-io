...
test.describe('Home Page', () => {
  ...
  test('should display exactly 3 social links in hero section', async ({
    page
  }) => {
  ...
  test('should have correct social link hrefs and aria-labels', async ({
    page
  }) => {
    const hero = page.locator('.hero')

    const githubLink = hero.locator(
      '.social-link[href="https://github.com/Xerrion"]'
    )
    await expect(githubLink).toBeVisible()
    await expect(githubLink).toHaveAttribute('aria-label', /github/i)

    const linkedinLink = hero.locator(
      '.social-link[href="https://www.linkedin.com/in/lasse-skovgaard-nielsen/"]'
    )
    await expect(linkedinLink).toBeVisible()
    await expect(linkedinLink).toHaveAttribute('aria-label', /linkedin/i)

    const emailLink = hero.locator(
      '.social-link[href="mailto:lasse@xerrion.dk"]'
    )
  ...
  test('should navigate to the about page when about nav card is clicked', async ({
    page
  }) => {
